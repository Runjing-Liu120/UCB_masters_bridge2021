...
We introduce the notion of a random variable.
We saw several examples
already in the previous session:
the sum of two dice roles, the time until
radioactive decay, the change in the stock price.

Informally, a random variable $X$ is a variable
that can take a set of possible
values, called the \textit{range} of $X$,
and the possible values in its range
...
$X = x$? Or $X \leq x$? Or $a \leq X \leq b$? And so forth. We return to the same examples
...
\begin{example}[Rolling two dice]
  ...
  We computed yesterday that if
  the die are fair, the probability of rolling a 5 is
  \begin{align*}
